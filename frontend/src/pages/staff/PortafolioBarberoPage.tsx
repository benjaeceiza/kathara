...
const PortafolioBarberoPage: React.FC = () => {
    ...
    // Aquí levantamos el avatar de forma segura (sea del objeto poblado o plano)
    ...
    const especialidades = portafolio.especialidades && portafolio.especialidades.length > 0 
        ...
        : (barbero.especialidades || ['Master Barber']);
    ...
    return (
        <div className="min-h-screen bg-[#09090B] text-white animate-fadeIn">
            ...
            <div className="relative h-72 w-full overflow-hidden bg-zinc-950">
                {portada ? (
                    <img src={portada} alt="Portada" className="w-full h-full object-cover opacity-70" />
                ) : (
            ...
            <main className="max-w-5xl mx-auto px-6 -mt-24 relative z-10 pb-20">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6 mb-8">
                    <div className="w-36 h-36 rounded-3xl border-4 border-[#09090B] bg-zinc-800 shadow-2xl overflow-hidden shrink-0 flex items-center justify-center">
                        {avatar ? (
                    ...
                    <div className="text-center sm:text-left pb-2">
                        ...
                        <p className="text-orange-500 font-bold uppercase tracking-widest text-xs mt-1">Estilista</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <p className="text-zinc-400 leading-relaxed text-sm whitespace-pre-line">{biografia}</p>

                        <div className="space-y-4">
                            <h4 className="font-black text-white text-sm flex items-center gap-2">
                                <Scissors className="w-4 h-4 text-orange-500" /> Especialidades
                            </h4>
                            <div className="flex flex-wrap gap-2">
                                {especialidades.map((e: string, idx: number) => (
                                    <span key={idx} className="px-3 py-1.5 rounded-xl bg-zinc-900 border border-white/5 text-xs font-bold text-zinc-300">
                                        {e}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="space-y-4">
                        <div className="p-5 rounded-2xl bg-zinc-900/50 border border-white/5 space-y-3">
                            <div className="flex items-center gap-2 text-zinc-400">
                                ...
                                <span className="text-xs font-bold uppercase">Atención</span>
                            </div>
                            <p className="text-sm font-medium text-zinc-300">{horarios}</p>
                        </div>

                        {/* BOTONES SOCIALES */}
                        <div className="flex gap-3">
                            {whatsapp && (
                                <a href={`https://wa.me/${whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center py-3 bg-green-600 hover:bg-green-500 rounded-xl transition-all shadow-lg shadow-green-600/20" title="Contactar por WhatsApp">
                            ...
                            {instagram && (
                                <a href={`https://instagram.com/${instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center py-3 bg-zinc-900 hover:bg-zinc-800 border border-white/5 rounded-xl transition-all" title="Ver Instagram">
                ...
                {/* BENTO COLLAGE DE FOTOS */}
                {galeria.length > 0 ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px]">
                        {galeria.map((foto: any, i: number) => (
                            <div key={foto.public_id || i} className={`group overflow-hidden rounded-2xl border border-white/5 bg-zinc-950 ${i === 0 ? 'col-span-2 row-span-2' : ''}`}>
                    ...
                    <div className="py-12 text-center bg-zinc-900/30 rounded-3xl border border-white/5">
                        <p className="text-zinc-500 text-sm">Este profesional aún no cargó fotos en su galería.</p>
                    </div>
                )}
                ...
                <div className="mt-20 bg-gradient-to-r from-orange-500 to-amber-500 rounded-[2rem] p-10 flex flex-col sm:flex-row items-center justify-between gap-6 text-black">
                    ...
                    <Link to={`/reservar?peluquero=${id}`} className="px-8 py-4 bg-black text-white rounded-xl font-bold hover:bg-zinc-900 transition-all shadow-xl">
                        Reservar Turno
                    </Link>
                </div>

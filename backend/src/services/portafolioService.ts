...
export const obtenerPortafolio = async (peluqueroId: string) => {
  const portafolio = await Portafolio.findOne({ peluquero: peluqueroId })
    .populate('peluquero', 'nombre apellido avatar telefono especialidades'); // 🔥 Esto trae el objeto del usuario completo
...
export const actualizarDatosPortafolio = async (peluqueroId: string, datos: any) => {
  if (datos.redesProfesionales?.whatsapp) {
    await Usuario.findByIdAndUpdate(peluqueroId, {
      telefono: datos.redesProfesionales.whatsapp
    });
  }
  ...
  return await Portafolio.findOneAndUpdate(
    ...
    { 
      $set: {
        ...
        estilo: datos.estilo,
        horarios: datos.horarios // 🔥 Guardamos los horarios en la BD
      }
    },
    { returnDocument: 'after', upsert: true }
  ).populate('peluquero', 'nombre apellido avatar telefono especialidades');
};
...
export const agregarFotoGaleria = async (peluqueroId: string, fileBuffer: Buffer) => {
    ...
    return await Portafolio.findOneAndUpdate(
        { peluquero: peluqueroId },
        { $push: { galeria: { url: resultado.secure_url, public_id: resultado.public_id } } },
        { returnDocument: 'after' }
    );
};
